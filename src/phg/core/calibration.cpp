...
cv::Vec3d phg::Calibration::project(const cv::Vec3d &point) const
{
    ...
    double r2 = x * x + y * y;
    double dist = 1.0 + r2 * k1_ + r2 * r2 * k2_;
    x *= dist;
    y *= dist;
...
cv::Vec3d phg::Calibration::unproject(const cv::Vec2d &pixel) const
{
    ...
    double xUnprojected = x;
    double yUnprojected = y;
    double eps = 1e-10;
    for (size_t i = 0; i < 100; ++i) {
        double r2 = xUnprojected * xUnprojected + yUnprojected * yUnprojected;
        double dist = 1.0 + r2 * k1_ + r2 * r2 * k2_;

        double delta = std::max(std::abs(xUnprojected * dist - x), std::abs(yUnprojected * dist - y));
        if (delta <= eps) {
            break;
        }
    ...
    return cv::Vec3d(xUnprojected, yUnprojected, 1.0);
}
