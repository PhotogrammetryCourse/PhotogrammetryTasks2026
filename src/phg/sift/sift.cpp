...
std::vector<phg::SIFT::Octave> phg::buildOctaves(const cv::Mat& img, const phg::SIFTParams& p, int verbose_level)
{
    ...
    for (int o = 0; o < n_octaves; o++) {
        ...
        for (int i = 1; i < n_layers; i++) {
            // корень из двух нужной степени, чтобы при i==s получали удвоение базового блюра
            double sigma_layer = sigma0 * std::pow(2.0, i * 1.0 / s);
        ...
        if (o + 1 < n_octaves) {
            ...
            cv::resize(oct.layers[s], base, cv::Size(0, 0), 0.5, 0.5, cv::INTER_NEAREST);
        ...
        for (int layer = 1; layer <= s; layer++) {
            ...
            for (int y = border; y < rows - border; y++) {
                ...
                for (int x = border; x < cols - border; x++) {
                    ...
                    const float* ptrs[3][3] = {{pp, p, pn}, {cp, c, cn}, {np, n, nn}};
...
void phg::SIFT::detectAndCompute(const cv::Mat& img, const cv::Mat& mask, std::vector<cv::KeyPoint>& kpts, cv::Mat& desc) const
{
    ...
    // Нет, мы можем получить меньше точек, если они в целом не нашлись
    // Если мы получили меньше точек, то можно запустить алгоритм с меньшим threshold. Либо заранее задать несколько порогов, либо просто каждый раз умножать на константу
}
