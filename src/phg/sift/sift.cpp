...
std::vector<phg::SIFT::Octave> phg::buildOctaves(const cv::Mat& img, const phg::SIFTParams& p, int verbose_level)
{
    ...
    for (int o = 0; o < n_octaves; o++) {
        ...
        for (int i = 1; i < n_layers; i++) {
            ...
            double sigma_layer_abs = sigma_prev * std::pow(2.f, 1.f / s);
            // вычтем sigma0 чтобы размыть ровно до нужной суммарной сигмы
            // вычитаем как в sigma base
            double sigma_layer = std::sqrt(sigma_layer_abs * sigma_layer_abs - sigma_prev * sigma_prev);
            cv::GaussianBlur(oct.layers[i - 1], oct.layers[i], cv::Size(), sigma_layer, sigma_layer);
            sigma_prev = sigma_layer_abs;
        }
                    ...
                    for (int step = 0; step < max_interp_steps; step++) {
                        ...
                        if (std::abs(offset[0]) < 0.5f && std::abs(offset[1]) < 0.5f && std::abs(offset[2]) < 0.5f) {
                            ...
                            if (params.enable_edge_like_filtering) {
                                ...
                                float expected_r = edge_threshold;
                                if (trace * trace / det > (expected_r + 1)*(expected_r + 1) / expected_r) break;
                            }
    ...
    for (const cv::KeyPoint& kp : kpts) {
        ...
        for (int dy = -radius; dy <= radius; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                ...
                histogram[bin0] += mag * weight * (1 - frac);
