...
std::vector<phg::SIFT::Octave> phg::buildOctaves(const cv::Mat& img, const phg::SIFTParams& p, int verbose_level)
{
    ...
    for (int o = 0; o < n_octaves; o++) {
        ...
        for (int i = 1; i < n_layers; i++) {
            ...
            double sigma_layer = sigma0 * std::pow(2, (double)i / s);
        ...
        for (int layer = 1; layer <= s; layer++) {
            ...
            for (int y = border; y < rows - border; y++) {
                ...
                for (int x = border; x < cols - border; x++) {
                    ...
                    check(cp[x]); check(cp[x+1]); check(cp[x-1]);
                    check(cn[x]); check(cn[x+1]); check(cn[x-1]);

                    if (!is_max && !is_min)
                        continue;

                    // TODO проверить локальный максимум на предыдущем скейле
                    check(p[x]); check(p[x+1]); check(p[x-1]);
                    check(pp[x]); check(pp[x+1]); check(pp[x-1]);
                    check(pn[x]); check(pn[x+1]); check(pn[x-1]);

                    if (!is_max && !is_min)
                        continue;

                    // TODO проверить локальный максимум на следующем скейле
                    check(n[x]); check(n[x+1]); check(n[x-1]);
                    check(np[x]); check(np[x+1]); check(np[x-1]);
                    check(nn[x]); check(nn[x+1]); check(nn[x-1]);
    ...
    for (const cv::KeyPoint& kp : kpts) {
        ...
        for (int dy = -radius; dy <= radius; dy++) {
            for (int dx = -radius; dx <= radius; dx++) {
                ...
                // голосуем в гистограмме направлений. находим два ближдайших бина и гладко распрееляем голос между ними
        ...
        for (int i = 0; i < n_bins; i++) {
            ...
            if (histogram[i] > histogram[prev] && histogram[i] > histogram[next] && histogram[i] >= peak_ratio * max_val) {
                ...
                float offset = (std::abs(left - 2.f * center + right) > 1e-10f) ? (0.5f * (left - right) / (left - 2.f * center + right)) : 0.f;
                    ...
                    for (int diy = 0; diy <= 1; diy++) {
                        ...
                        for (int dix = 0; dix <= 1; dix++) {
                            ...
                            for (int dio = 0; dio <= 1; dio++) {
                                ...
                                desc[idx] += wx * wy * wo * weighted_mag;
                            }
                        }
                    }
                } else {
