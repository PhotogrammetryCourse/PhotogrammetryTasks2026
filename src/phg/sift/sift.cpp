...
std::vector<phg::SIFT::Octave> phg::buildOctaves(const cv::Mat& img, const phg::SIFTParams& p, int verbose_level)
{
    ...
    for (int o = 0; o < n_octaves; o++) {
        ...
        if (o + 1 < n_octaves) {
            ...
            // даунскейлим текущий слой в два раза, без интерполяции, просто сабсеrпмлинг
            ...
            for (int y = border; y < rows - border; y++) {
                ...
                for (int x = border; x < cols - border; x++) {
                    ...
                    for (int dy = -1; dy <= 1; ++dy) {
                        const float* row = (dy == -1) ? cp : (dy == 0 ? c : cn);
                    ...
                    for (int dy = -1; dy <= 1; ++dy) {
                        const float* row = (dy == -1) ? pp : (dy == 0 ? p : pn);
                        for (int dx = -1; dx <= 1; ++dx) {
                            float v = row[x + dx];
                            if (v >= val) is_max = false;
                            if (v <= val) is_min = false;
                    ...
                    for (int dy = -1; dy <= 1; ++dy) {
                        const float* row = (dy == -1) ? np : (dy == 0 ? n : nn);
                        for (int dx = -1; dx <= 1; ++dx) {
                            float v = row[x + dx];
                            if (v >= val) is_max = false;
                            if (v <= val) is_min = false;
...
void phg::SIFT::detectAndCompute(const cv::Mat& img, const cv::Mat& mask, std::vector<cv::KeyPoint>& kpts, cv::Mat& desc) const
{
    ...
    //
    // не всегда, может быть меньше (но больше нет из-за selectTopKeypoints), в случае когда на картинке просто много фичей. 
