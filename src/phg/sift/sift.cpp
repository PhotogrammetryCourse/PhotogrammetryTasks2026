...
//                        float dys = TODO;
                        ...
                        if (std::abs(offset[0]) < 0.5f && std::abs(offset[1]) < 0.5f && std::abs(offset[2]) < 0.5f) {
                            ...
                            if (params.enable_edge_like_filtering) {
                                ...
                                if (trace * trace * r >= (r + 1.f) * (r + 1.f) * det)
                                    break;
...
//                oriented_kpts.push_back(new_kp);
                float denom = left + right - 2.f * center;
                float offset = (std::abs(denom) > 1e-7f) ? (0.5f * (left - right) / denom) : 0.f;
                if (!params.enable_orientation_subpixel_localization) {
                    offset = 0.f;
                }
                offset = std::max(-1.f, std::min(1.f, offset));
