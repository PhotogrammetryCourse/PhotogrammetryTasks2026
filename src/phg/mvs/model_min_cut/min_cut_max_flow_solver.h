...
template <typename node_t> class MaxFlow {
...
public:
    MaxFlow(size_t numNodes)
        : graph((int)numNodes, 4 * (int)numNodes)
        , numNodes(numNodes)
    {
    }
    ...
    value_t computeMaxFlow() { return graph.maxflow(); }

    inline bool isNodeOnSrcSide(node_t n) { return (graph.what_segment(n) == graph_t::SOURCE); }
