...
vector3d unproject(const vector3d& pixel, const phg::Calibration& calibration, const matrix34d& PtoWorld)
{
    ...
    // TODO 102 пустите луч pixel из calibration а затем возьмите на нем точку у которой по оси +Z координата=depth
    // phg::Calibration::unproject возвращает cv::Vec3d(x, y, 1.0);
    ...
    vector3d global_point = PtoWorld * vector4d(local_point[0], local_point[1], local_point[2], 1.0);
    ...
    for (ptrdiff_t j = 0; j < height; ++j) {
        for (ptrdiff_t i = 0; i < width; ++i) {
...

            {
                ...
                np = cv::normalize(n0 + randomNormalObservedFromCamera(cameras_RtoWorld[ref_cam], r) * p);
        ...
        for (ptrdiff_t j = 0; j < height; ++j) {
            for (ptrdiff_t i = (j + chessboard_pattern_step) % 2; i < width; i += 2) {
                ...
                std::vector<std::vector<std::pair<int, int>>> patterns = { 
                    { { i - 1, j + 0 }, { i - 2, j + 1 }, { i - 2, j - 1 } }, 
                    { { i + 0, j - 1 }, { i - 1, j - 2 }, { i + 1, j - 2 } }, 
                    { { i + 1, j + 0 }, { i + 2, j - 1 }, { i + 2, j + 1 } },
                    { { i + 0, j + 1 }, { i + 1, j + 2 }, { i - 1, j + 2 } }, 
                    { { i - 3, j + 0 }, { i - 1 * PROPAGATION_STEP, j + 0 * PROPAGATION_STEP } }, 
                    { { i + 0, j - 3 }, { i + 0 * PROPAGATION_STEP, j - 1 * PROPAGATION_STEP } },
                    { { i + 3, j + 0 }, { i + 1 * PROPAGATION_STEP, j + 0 * PROPAGATION_STEP } }, 
                    { { i + 0, j + 3 }, { i + 0 * PROPAGATION_STEP, j + 1 * PROPAGATION_STEP } } 
                };

                for (auto combination : patterns) {
                    double best_cost = -1;
                    double best_x = -1;
                    double best_y = -1;

                    for (auto indexes : combination) {
                        int x = indexes.first;
                        int y = indexes.second;
                        double cost = cost_map.at<float>(y, x);
                    ...
                    tryToPropagateDonor(best_x, best_y, chessboard_pattern_step, hypos_depth, hypos_normal, hypos_cost);
                }
...
float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d& global_normal, size_t neighb_cam)
{
    ...
    for (ptrdiff_t dj = -COST_PATCH_RADIUS; dj <= COST_PATCH_RADIUS; ++dj) {
        for (ptrdiff_t di = -COST_PATCH_RADIUS; di <= COST_PATCH_RADIUS; ++di) {
            ...
            double x0 = std::floor(x - 0.5);
            double y0 = std::floor(y - 0.5);
            double x1 = x0 + 1.0;
            double y1 = y0 + 1.0;
            ...
            float val00 = cameras_imgs_grey[neighb_cam].at<unsigned char>(y0, x0) / 255.0f;
            float val01 = cameras_imgs_grey[neighb_cam].at<unsigned char>(y0, x1) / 255.0f;
            float val10 = cameras_imgs_grey[neighb_cam].at<unsigned char>(y1, x0) / 255.0f;
            float val11 = cameras_imgs_grey[neighb_cam].at<unsigned char>(y1, x1) / 255.0f;

            float a = (float)(x1 - x) / (float)(x1 - x0);
            float b = (float)(x - x0) / (float)(x1 - x0);
            float c = (float)(y1 - y) / (float)(y1 - y0);
            float d = (float)(y - y0) / (float)(y1 - y0);

            float r1 = a * val00 + b * val10;
            float r2 = a * val01 + b * val11;
            float intensity = c * r1 + d * r2;
    ...
    // sigma0 = √(∑_i(patch0[i] - mean0)^2)
    // sigma1 = √(∑_i(patch1[i] - mean1)^2)
    // ZNCC(patch0, patch1) = (∑_i(patch0[i] - mean0)*(patch1[i] - mean1)) / (sigma0 * sigma1)
    ...
    for (size_t k = 0; k < n; k++) {
        float a = patch0[k];
        float b = patch1[k];
        sigma0 += (a - mean0) * (a - mean0);
        sigma1 += (b - mean1) * (b - mean1);
        zncc += (a - mean0) * (b - mean1);
    }

    if (sigma0 < 10e-9 || sigma1 < 10e-9)
        return NO_COST;
...
float PMDepthMapsBuilder::avgCost(std::vector<float>& costs)
{
    ...
    float limit = std::min(best_cost, 1.0f) * COSTS_K_RATIO;
    float count = std::min((int)costs.size(), COSTS_BEST_K_LIMIT);
