    ...
    for (ptrdiff_t j = 0; j < height; ++j) {
        for (ptrdiff_t i = 0; i < width; ++i) {
...

            {
                ...
                const double norm = static_cast<double>(NITERATIONS - iter) / NITERATIONS;
                dp = r.nextf(d0 * (1.0 - 0.5 * norm), d0 * (1.0 + 0.5 * norm)); // TODO 104: сделайте так чтобы отклонение было тем меньше, чем номер итерации ближе к NITERATIONS, улучшило ли это результат?
                np = cv::normalize(n0 + randomNormalObservedFromCamera(cameras_RtoWorld[ref_cam], r) * 0.5 * norm);
...
float PMDepthMapsBuilder::estimateCost(ptrdiff_t i, ptrdiff_t j, double d, const vector3d& global_normal, size_t neighb_cam)
{
    ...
    for (ptrdiff_t dj = -COST_PATCH_RADIUS; dj <= COST_PATCH_RADIUS; ++dj) {
        for (ptrdiff_t di = -COST_PATCH_RADIUS; di <= COST_PATCH_RADIUS; ++di) {
            ...
            float intensity = (1-dx)*(1-dy)*cameras_imgs_grey[neighb_cam].at<unsigned char>(py, px) +
                (1-dx)*dy*cameras_imgs_grey[neighb_cam].at<unsigned char>(py + 1, px) +
                    dx*(1-dy)*cameras_imgs_grey[neighb_cam].at<unsigned char>(py, px + 1) +
                        dx*dy*cameras_imgs_grey[neighb_cam].at<unsigned char>(py + 1, px + 1);
            intensity /= 255.f;
...
float PMDepthMapsBuilder::avgCost(std::vector<float>& costs)
{
    ...
    // взвешенное среднее из лучших COSTS_BEST_K_LIMIT

    int n = std::min(COSTS_BEST_K_LIMIT, static_cast<int>(costs.size()));
    ...
    for (int i = 0; i < n; i++) {
        ...
        float weight = 1.0f / (0.01f + costs[i]);
        cost_sum += costs[i] * weight;
        cost_w += weight;
    }
