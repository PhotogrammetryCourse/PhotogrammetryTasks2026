...
namespace {
    ...
    void ensureSpectralProperty(matrix3d &Ecv)
    {
        ...
        // два ненулевых сингулярных числа должны быть равны, третье = 0
        double s = (svd.singularValues()[0] + svd.singularValues()[1]) / 2.0;
        Eigen::Vector3d sv(s, s, 0.0);
        E = svd.matrixU() * sv.asDiagonal() * svd.matrixV().transpose();
...
namespace {
    ...
    {
        // перевести пиксели в нормализованные лучи (убрать K)
        ...
        // глубина в камере 0: просто Z (P0 = I|0, мировые координаты = координаты камеры 0)
        double depth0 = X[2];

        // глубина в камере 1: Z-компонента после проекции через P1
        vector3d proj1 = P1 * X;
        double depth1 = proj1[2];
...
void phg::decomposeEMatrix(cv::Matx34d &P0, cv::Matx34d &P1, const cv::Matx33d &Ecv, const std::vector<cv::Vec2d> &m0, const std::vector<cv::Vec2d> &m1, const Calibration &calib0, const Calibration &calib1)
{
    ...
    // W — матрица поворота на 90 градусов
    mat W(3, 3);
    W << 0, -1, 0,
         1,  0, 0,
         0,  0, 1;
    ...
    // выбрать решение где 3D-точки перед обеими камерами (положительная глубина)
    int best_count = 0;
    int best_idx = -1;
    for (int i = 0; i < 4; ++i) {
        int count = 0;
