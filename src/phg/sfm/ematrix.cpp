...
void ensureSpectralProperty(matrix3d& Ecv)
{
    ...
    double avg_sing = (singular_vals(0) + singular_vals(1)) / 2.0;
    Eigen::MatrixXd sigma = Eigen::MatrixXd::Zero(3, 3);
    sigma(0, 0) = avg_sing;
    sigma(1, 1) = avg_sing;

    E = svd.matrixU() * sigma * svd.matrixV().transpose();
...
bool depthTest(const vector2d& m0, const vector2d& m1, const phg::Calibration& calib0, const phg::Calibration& calib1, const matrix34d& P0, const matrix34d& P1)
{
    ...
    auto P = vector3d(X[0], X[1], X[2]);

    // точка должна иметь положительную глубину для обеих камер
    return calib0.project(P)[2] > 0 && calib1.project(P)[2] > 0;
}
}
...
cv::Matx33d phg::composeEMatrixRT(const cv::Matx33d& R, const cv::Vec3d& T) { return skew(T) * R; }
