...
namespace {
    ...
    void ensureSpectralProperty(matrix3d &Ecv)
    {
        ...
        double mean = (svd.singularValues()[0] + svd.singularValues()[1]) / 2;
        diag << mean, 0, 0, 0, mean, 0, 0, 0, 0; 
        E = svd.matrixU() * diag * svd.matrixV().transpose();
...
namespace {
    ...
    bool depthTest(const vector2d &m0, const vector2d &m1, const phg::Calibration &calib0, const phg::Calibration &calib1, const matrix34d &P0, const matrix34d &P1)
    {
        ...
        // TODO
        cv::Vec3d px(X[0], X[1], X[2]);
        double depth0 = calib0.project(px)[2];
        double depth1 = calib1.project(px)[2];
