...
{
    cv::Vec3d p0(pt0[0], pt0[1], 1);
    cv::Vec3d p1(pt1[0], pt1[1], 1);
    auto normal = F * p0;
    auto dot = p1.dot(normal);
    auto normal_dist = std::sqrt(normal[0] * normal[0] + normal[1] * normal[1]);
    return normal_dist > 0 && std::abs(dot) < normal_dist * t;
}
