...
{
    const auto l = F * cv::Vec3d(pt0[0], pt0[1], 1.0);

    const double n_sq = l[0] * l[0] + l[1] * l[1];
    if (n_sq < 1e-12) {
        return false;
    }

    const double numerator = l[0] * pt1[0] + l[1] * pt1[1] + l[2];
    const double distance = std::abs(numerator) / std::sqrt(n_sq);
