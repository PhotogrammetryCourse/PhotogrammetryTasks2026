...
//            std::cout << "gauss: unique solution found" << std::endl;
    ...
    cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs)
    {
        ...
        const int n_trials = std::ceil(std::log(1 - p) / std::log(1 - std::pow(w, n_samples)));
        uint64_t seed = 1;
...
{
    rassert(T.rows == 3 && T.cols == 3, 010101);
    cv::Mat out = T * cv::Mat(cv::Point3d(pt.x, pt.y, 1.0));
    return {out.at<double>(0) / out.at<double>(2), out.at<double>(1)/out.at<double>(2)};
}
