...
{
    double x = pt.x * T.at<double>(0, 0) + pt.y * T.at<double>(0, 1) + T.at<double>(0, 2);
    double y = pt.x * T.at<double>(1, 0) + pt.y * T.at<double>(1, 1) + T.at<double>(1, 2);
    double w = pt.x * T.at<double>(2, 0) + pt.y * T.at<double>(2, 1) + T.at<double>(2, 2);
    rassert(w != 0, 3290490321);
    return {x / w, y / w};
}
