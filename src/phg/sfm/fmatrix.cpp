...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        Eigen::MatrixXd U = svdf.matrixU();
        Eigen::MatrixXd V = svdf.matrixV();
        Eigen::MatrixXd singular = svdf.singularValues().asDiagonal();
        singular(2, 2) = 0;

        F = U * singular * V.transpose();
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        double sum = 0.0f;
        for (auto&& pt : m) {
            cv::Vec2d d = pt - centroid;
            sum += d.dot(d);
        }

        double rms = std::sqrt(sum / m_size);
        double scale = std::sqrt(2) / rms;
        return cv::Matx33d(scale, 0, -scale*centroid[0], 0, scale, -scale*centroid[1], 0, 0, 1);
    }
