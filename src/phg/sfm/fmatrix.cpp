...
namespace {
    ...
    cv::Matx33d estimateFMatrixDLT(const cv::Vec2d *m0, const cv::Vec2d *m1, int count)
    {
        ...
        Sigma(0, 0) = svdf.singularValues()(0);
        Sigma(1, 1) = svdf.singularValues()(1);
        F = svdf.matrixU() * Sigma * svdf.matrixV().transpose();
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        double dist2 = 0;
        for (auto& v : m) {
            dist2 += (mean - v).dot(mean - v);
        }
        dist2 /= (double)m.size();
        double scale = std::sqrt(2.0) / dist2;
        return cv::Matx33d{
