...
namespace {
    ...
    //             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        auto diag = svdf.singularValues();
        diag[2] = 0.0;
        F = svdf.matrixU() * diag.asDiagonal() * svdf.matrixV().transpose();
    ...
    {
        cv::Vec2d centroid = std::accumulate(m.begin(), m.end(), cv::Vec2d()) / static_cast<double>(m.size());
        double rms2 = 0.0;
        for (const auto& p : m) {
            rms2 += cv::norm(p - centroid, cv::NORM_L2SQR);
        } // сюды бы std::ranges...

        double scale = std::sqrt(2.0 * m.size() / rms2);
        return {
