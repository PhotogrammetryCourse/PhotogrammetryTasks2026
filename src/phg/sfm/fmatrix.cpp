...
namespace {
    ...
    {
        // каждая пара точек даёт одно уравнение: x1^T * F * x0 = 0
        // раскрываем → строка матрицы A: [x1*x0, x1*y0, x1, y1*x0, y1*y0, y1, x0, y0, 1]
        ...
        // null space A → последний столбец V
        ...
        // принудить ранг 2: SVD, обнулить наименьшее сингулярное число
        Eigen::JacobiSVD<Eigen::MatrixXd> svdf(F, Eigen::ComputeFullU | Eigen::ComputeFullV);
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        // центр масс
        cv::Vec2d mean(0, 0);
        for (const auto &p : m) mean += p;
        mean /= (double)m.size();

        // RMS расстояние от центра масс
        double rms = 0;
        for (const auto &p : m) {
            double dx = p[0] - mean[0], dy = p[1] - mean[1];
            rms += dx*dx + dy*dy;
        }
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
        ...
        // для F нужно минимум 8 точек (степеней свободы больше, чем у гомографии)
        const int n_samples = 8;
        // число итераций из формулы RANSAC: гарантия 1-1e-6 при 30% инлаеров
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            // денормализация: F_real = T1^T * F_norm * T0
            F = TN1.t() * F * TN0;
            cv::Matx33d F_T = F.t();
            ...
            for (int i = 0; i < n_matches; ++i) {
                if (phg::epipolarTest(m0[i], m1[i], F, threshold_px) &&
                    phg::epipolarTest(m1[i], m0[i], F_T, threshold_px))
                    ++support;
            }
