...
namespace {
    ...
    //             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        Eigen::MatrixXd Uf = svdf.matrixU();
        Eigen::VectorXd sf = svdf.singularValues();
        Eigen::MatrixXd Vf = svdf.matrixV();

        sf[2] = 0.0;
        Eigen::MatrixXd Sf = Eigen::MatrixXd::Zero(3, 3);
        Sf(0, 0) = sf[0];
        Sf(1, 1) = sf[1];
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            cv::Matx33d F_norm = estimateFMatrixDLT(ms0, ms1, n_samples);
            cv::Matx33d F = TN1.t() * F_norm * TN0;
