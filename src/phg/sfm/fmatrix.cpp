...
//            std::cout << "(" << x0 << ", " << y0 << "), (" << x1 << ", " << y1 << ")" << std::endl;
            ...
            A.row(i_pair) << x1 * x0, x1 * y0, x1, y1 * x0, y1 * y0, y1, x0, y0, 1.0f;
        }
...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d>& m) {
        ...
        O /= static_cast<int>(m.size());
        double s = 0.0;
        for (auto& pt: m) {
            cv::Vec2d d = pt - O;
            s += d.dot(d);
        }
        s = std::sqrt(2.0 * static_cast<int>(m.size()) / s);
        return {
                s, 0, -s * O[0],
                0, s, -s * O[1],
                0, 0, 1
    ...
    cv::Matx33d
    estimateFMatrixRANSAC(const std::vector<cv::Vec2d>& m0, const std::vector<cv::Vec2d>& m1, double threshold_px) {
        if (m0.size() != m1.size()) {
