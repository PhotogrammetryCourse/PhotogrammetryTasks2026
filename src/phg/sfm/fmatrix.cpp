...
//            std::cout << "(" << x0 << ", " << y0 << "), (" << x1 << ", " << y1 << ")" << std::endl;
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        center /= static_cast<double>(m.size());

        double dist2Avg = 0.0;
        for (const auto& p: m) {
            dist2Avg += cv::norm(p - center, cv::NORM_L2SQR);
        }
        dist2Avg /= m.size();
        double scale = std::sqrt(2.0 / dist2Avg);
    ...
    cv::Matx33d estimateFMatrixRANSAC(const std::vector<cv::Vec2d> &m0, const std::vector<cv::Vec2d> &m1, double threshold_px)
    {
...

        {
            ...
            // std::cerr << getNormalizeTransform(m0_t) << std::endl;
            // std::cerr << getNormalizeTransform(m1_t) << std::endl;
        }
