...
//        throw std::runtime_error("not implemented yet");
        ...
        for (int i_pair = 0; i_pair < count; ++i_pair) {
            ...
            const std::vector<double> coef = {x1 * x0, x1 * y0, x1, y1 * x0, y1 * y0, y1, x0, y0, 1};
...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        auto singular_values = svdf.singularValues();
        singular_values[2] = 0;
...
//        DONE
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        double dist_avr = 0;
        for (auto& point: m) {
            dist_avr += (x_avr - point(0)) * (x_avr - point(0)) + (y_avr - point(1)) * (y_avr - point(1));
        }
        dist_avr /= n;
        double s = sqrt(2 / (dist_avr + 1e-8));
        return {s, 0,   -s * x_avr,
                0, s,   -s * y_avr,
                0, 0,           1};
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
            ...
            auto transform = getNormalizeTransform(m1_t);
            std::cout << transform(0, 0) << ' ' << transform(0, 1) << ' ' << transform(0, 2) << "\n";
            std::cout << transform(1, 0) << ' ' << transform(1, 1) << ' ' << transform(1, 2) << "\n";
            std::cout << transform(2, 0) << ' ' << transform(2, 1) << ' ' << transform(2, 2) << "\n";
        }
//        throw std::runtime_error("not implemented yet");
