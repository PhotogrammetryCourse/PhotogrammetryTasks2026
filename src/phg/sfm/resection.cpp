...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        Eigen::VectorXd null_space = svda.matrixV().col(A.cols() - 1);

        result << null_space[0], null_space[1], null_space[2], null_space[3],
    ...
    cv::Vec2d transformPoint(const cv::Vec3d &pt, const cv::Matx33d &T)
    {
        cv::Vec3d tmp = T * pt;

        if (tmp[2] == 0) {
            throw std::runtime_error("infinite point 33");
        }
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                cv::Vec2d px = transformPoint(P * cv::Vec4d(X[i][0], X[i][1], X[i][2], 1.0), calib.K());
