...
cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration& calib, const std::vector<cv::Vec3d>& X, const std::vector<cv::Vec2d>& x)
{
    ...
    for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
        ...
        for (int i = 0; i < n_points; ++i) {
            auto X_proj = calib.project(P * cv::Vec4d(X[i][0], X[i][1], X[i][2], 1.0));
            cv::Vec2d px = cv::Vec2d(X_proj[0], X_proj[1]) / X_proj[2];
...
cv::Matx34d phg::findCameraMatrix(const Calibration& calib, const std::vector<cv::Vec3d>& X, const std::vector<cv::Vec2d>& x) { return estimateCameraMatrixRANSAC(calib, X, x); }
