...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        Eigen::VectorXd ps = svd.matrixV().col(svd.matrixV().cols() - 1);
        for (int i = 0; i < 3; ++i) {
            for (int j = 0; j < 4; ++j) {
                result(i, j) = ps(4 * i + j);
            }
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                // TODO спроектировать 3Д точку в пиксель с использованием P и calib
                cv::Vec3d wpx = calib.project(P * cv::Vec4d(X[i][0], X[i][1], X[i][2], 1.0));
                cv::Vec2d px(wpx[0] / wpx[2], wpx[1] / wpx[2]);
