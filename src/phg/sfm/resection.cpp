...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            A.row(i * 2) << 0.0, 0.0, 0.0, 0.0, -w * X, -w * Y, -w * Z, -w * W, y * X,  y * Y,  y * Z,  y * W;
            A.row(i * 2 + 1) << w * X,  w * Y,  w * Z,  w * W, 0.0, 0.0, 0.0, 0.0, -x * X, -x * Y, -x * Z, -x * W;
        }
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        std::vector<cv::Vec3d> x_unproj(n_points);
        for (int i = 0; i < n_points; ++i)
            x_unproj[i] = calib.unproject(x[i]);

        if (n_points == n_samples)
            return estimateCameraMatrixDLT(X.data(), x_unproj.data(), n_points);
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            int support = 0;
            for (int i = 0; i < n_points; ++i) {
                cv::Vec3d cam = P * cv::Vec4d(X[i][0], X[i][1], X[i][2], 1.0);
                if (std::abs(cam[2]) < 1e-6) {
                    continue;
                }

                cv::Vec3d px_proj = calib.project(cam);
                cv::Vec2d px(px_proj[0] / px_proj[2], px_proj[1] / px_proj[2]);
            ...
            if (support > best_support) {
                ...
                std::cout << "estimateCameraMatrixRANSAC : support: " << best_support << "/" << n_points << ", i_trial: " << i_trial << std::endl;
