...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            auto fill_coef = [&](int ind_i, int ind_j, double coef){
                A(ind_i, ind_j) = coef * X;
                A(ind_i, ind_j + 1) = coef * Y;
                A(ind_i, ind_j + 2) = coef * Z;
                A(ind_i, ind_j + 3) = coef * W;
            };
            fill_coef(2 * i, 0, 1);
            fill_coef(2 * i, 8, -x);
            fill_coef(2 * i + 1, 4, 1);
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                cv::Vec4d xi = cv::Vec4d(X[i][0], X[i][1], X[i][2], 1.0);
                auto proj = calib.project(P * xi);
