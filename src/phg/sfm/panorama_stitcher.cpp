...
cv::Mat phg::stitchPanorama(const std::vector<cv::Mat>& imgs, const std::vector<int>& parent, std::function<cv::Mat(const cv::Mat&, const cv::Mat&)>& homography_builder)
{
    ...
    std::vector<cv::Mat> Hs(n_images);
    {
        ...
        if (root == -1) {
            throw std::runtime_error("Could not find root in given panorama tree");
        }
        std::vector<int> ready = { root };

        while (!ready.empty()) {
            int par = ready.back();
            ready.pop_back();
            for (int child : children[par]) {
                Hs[child] = homography_builder(imgs[child], imgs[par]) * Hs[par];
