...
void phg::DescriptorMatcher::filterMatchesRatioTest(const std::vector<std::vector<cv::DMatch>>& matches, std::vector<cv::DMatch>& filtered_matches)
{
    ...
    for (const auto& knn : matches) {
        if (knn.size() >= 2) {
            if (knn[0].distance < THRESHOLD * knn[1].distance) {
                filtered_matches.emplace_back(knn[0]);
            }
        } else if (knn.size() == 1) {
            filtered_matches.emplace_back(knn[0]);
        }
...
void phg::DescriptorMatcher::filterMatchesClusters(const std::vector<cv::DMatch>& matches, const std::vector<cv::KeyPoint> keypoints_query, const std::vector<cv::KeyPoint> keypoints_train, std::vector<cv::DMatch>& filtered_matches)
{
    ...
    for (std::size_t i = 0; i < n_matches; i++) {
        std::vector<int> neighbors_query;
        ...
        std::size_t intersect_size = 0;
        for (int j = 0; j < total_neighbours; ++j) {
            if (distances2_train.at<float>(i, j) <= radius2_train) {
                int neighbor_train = indices_train.at<int>(i, j);
                if (*std::lower_bound(neighbors_query.begin(), neighbors_query.end(), neighbor_train) == neighbor_train) {
                    if (++intersect_size >= consistent_matches) {
