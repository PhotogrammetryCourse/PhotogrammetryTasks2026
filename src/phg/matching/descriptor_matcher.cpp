                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (const auto &desc_matches : matches) {
        if (desc_matches.size() < 2)
            continue;

        const cv::DMatch &best = desc_matches[0];
        if (best.distance < ratio_threshold * desc_matches[1].distance)
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    std::shared_ptr<cv::flann::SearchParams> search_params = flannKsTreeSearchParams(std::max(32, n_matches));
    ...
    filtered_matches.reserve(matches.size());
    for (int i = 0; i < n_matches; ++i) {
        std::unordered_set<int> query_neighbours;
        query_neighbours.reserve(total_neighbours);
        ...
        for (size_t j = 0; j < total_neighbours; ++j) {
            if (distances2_train.at<float>(i, j) <= radius2_train && query_neighbours.count(indices_train.at<int>(i, j)))
                ++intersection_size;
        }
