                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (auto& knn: matches) {
        if (knn.size() >= 2 && knn[0].distance > knn[1].distance * ratio) {
            continue;
        }
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < n_matches; ++i) {
        std::unordered_set<int> in_search_query;
        for (int j = 0; j < total_neighbours; ++j) {
            if (distances2_query.at<float>(i, j) <= radius2_query) {
                in_search_query.insert(indices_query.at<int>(i, j));
            }
        }
        int common = 0;
        for (int j = 0; j < total_neighbours; ++j) {
            if (distances2_train.at<float>(i, j) <= radius2_train &&
                 in_search_query.find(indices_train.at<int>(i, j)) != in_search_query.end()) {
                common++;
            }
        }
