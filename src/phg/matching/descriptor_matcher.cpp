                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (int i = 0; i < n_matches; ++i) {
        ...
        // собираем множество индексов соседей в train-пространстве (в пределах радиуса)
        std::set<int> neighbours_train;
        for (size_t j = 0; j < total_neighbours; ++j) {
        ...
        size_t n_consistent = 0;
        for (int idx : neighbours_query) {
            if (neighbours_train.count(idx)) {
                ++n_consistent;
            }
        }
