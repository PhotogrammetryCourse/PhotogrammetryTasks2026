...
{
    const float ratio_threshold = 0.55f;
    ...
    for (int i = 0; i < n_matches; ++i) {
        ...
        for (int j = 0; j != total_neighbours; ++j) {
            if (distances2_train.at<float>(i, j) <= radius2_train) {
                if (query_nn.count(indices_train.at<int>(i, j)) > 0) {
                    ++intersection_cnt;
                }
            }
        }
        if (intersection_cnt >= consistent_matches) {
