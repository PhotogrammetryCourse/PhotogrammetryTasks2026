...
void filterMatchesF(const std::vector<cv::DMatch>& matches, const std::vector<cv::KeyPoint> keypoints_query, const std::vector<cv::KeyPoint> keypoints_train, const cv::Matx33d& F, std::vector<cv::DMatch>& result, double threshold_px)
{
    result.clear();
...
TEST(SFM, TriangulationSimple)
{
    ...
    vector3d T = -R * O;
    matrix34d P1 = { R(0, 0), R(0, 1), R(0, 2), T[0], R(1, 0), R(1, 1), R(1, 2), T[1], R(2, 0), R(2, 1), R(2, 2), T[2] };
