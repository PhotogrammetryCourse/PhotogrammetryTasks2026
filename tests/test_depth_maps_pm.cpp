...
/*TEST(test_depth_maps_pm, AllFullDepthMaps)
{
    Dataset full_dataset = loadDataset(DATASET_DIR, DATASET_DOWNSCALE);
    ...
    for (size_t ref = 0; ref < full_dataset.ncameras; ++ref) {
        const Dataset &dataset = full_dataset;

        phg::PMDepthMapsBuilder builder(dataset.ncameras, dataset.cameras_imgs, dataset.cameras_imgs_grey, dataset.cameras_labels, dataset.cameras_P, dataset.calibration);
        cv::Mat depth_map, normal_map, cost_map;
        builder.buildDepthMap(ref, depth_map, normal_map, cost_map, dataset.cameras_depth_min[ref], dataset.cameras_depth_max[ref]);
        phg::PMDepthMapsBuilder::buildGoodPoints(depth_map, normal_map, cost_map, dataset.cameras_imgs[ref], dataset.calibration, builder.getCameraPtoWorld(ref), all_points, all_colors, all_normals);
        ++ndepth_maps;
