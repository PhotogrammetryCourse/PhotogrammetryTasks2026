...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        for (int i = 0; i < 3; ++i) {
            point[i] = point_global[i] - camera_extrinsics[i];
        }
        // rotation[3] - angle-axis rotation, поворачиваем точку point->p (чтобы перейти в локальную систему координат камеры)
        T point_rotated[3];
        ceres::AngleAxisRotatePoint(camera_extrinsics + 3, point, point_rotated);
    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
                for (int i = 0; i < ci; ++i) {
                    int prev_id = track.img_kpt_pairs[i].first;
                    matrix3d R2;
                    vector3d prev_origin;
                    phg::decomposeUndistortedPMatrix(R2, prev_origin, cameras[prev_id]);
                    auto v1 = (track_point - camera_origin);
                    auto v2 = (track_point - prev_origin);
                    if (v1.dot(v2) / std::sqrt(v1.dot(v1) * v2.dot(v2)) > 0.99) {
                        should_be_disabled = true;
                        break;
                    }
                }
