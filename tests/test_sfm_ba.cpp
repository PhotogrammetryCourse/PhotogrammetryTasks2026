...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        T pts[3], pts_swap[3];
        for (int d = 0; d < 3; ++d) {
            pts[d] = point_global[d] - camera_extrinsics[d];
        }
        ...
        ceres::AngleAxisRotatePoint(camera_extrinsics + 3, pts, pts_swap);
        std::swap(pts, pts_swap);
        ...
        residuals[0] = pts[0] - observed[0];
        residuals[1] = pts[1] - observed[1];
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
                ...
                for (int pi = 0; pi < ci; ++pi) {
                    ...
                    matrix3d R_prev;
                    vector3d prev_origin;
                    phg::decomposeUndistortedPMatrix(R_prev, prev_origin, cameras[prev_id]);
