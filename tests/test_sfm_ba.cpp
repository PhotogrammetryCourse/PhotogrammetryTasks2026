    ...
    for (size_t i = 0; i < tie_points.size(); ++i) {
        ...
        for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
            ...
            if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
                vector3d dir = track_point - camera_origin;
                double dir_len = cv::norm(dir);
                if (dir_len > 0) {
                    dir /= dir_len;
                    for (size_t ck = 0; ck < track.img_kpt_pairs.size(); ++ck) {
                        int other_cam = track.img_kpt_pairs[ck].first;
                        if (other_cam == camera_id) {
                            continue;
                        }

                        matrix3d R_other; vector3d origin_other;
                        phg::decomposeUndistortedPMatrix(R_other, origin_other, cameras[other_cam]);
                        vector3d dir_other = track_point - origin_other;
                        double len_other = cv::norm(dir_other);

                        if (len_other > 0) {
                            dir_other /= len_other;
                            double cos_val = std::max(-1.0, std::min(1.0, dir.dot(dir_other)));

                            if (acos(cos_val) < 2.5 * M_PI / 180.0) {
                                should_be_disabled = true;
                                break;
                            }
                        }
