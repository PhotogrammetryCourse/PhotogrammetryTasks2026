...
// Ну вообще говоря она не меняется) Если мы сделали фотку, а потом просто уменьшили получившуюся картинку, то линза никак не могла измениться при это,
//   и фокальная длина в миллиметрах осталась тем же.
// Но в этом то и суть, что здесь фокальная длина измеряется не в миллиметрах, а в пикселях. А выражая фокальную длину в пикселях, мы фактически подвязываемся
//   уже не только на параметры оптики, но и на параметры конкретного изображения. В том числе и на масштаб этого изображения.
// Фокальная длина в пикселях - это по сути "сколько пикселей соответствует физической фокальной длине", поэтому очевидно, что эта фокальная длина прямо
//   пропорциональная масштабу изображения и уменьшается при увеличении степени downscale-а.
// А делить на downscale^2 нам не зачем --- у нас же длина - это не какая-то там площадь, чтобы быть пропорциональной квадрату масштаба :)
...
public:
    ...
    bool operator()(const T* camera_extrinsics, // положение камеры:   [6] = {translation[3], rotation[3]} (разное для всех кадров, т.к. каждая фотография со своего ракурса)
        ...
        T point_global_from_camera_center[3] = {
        ...
        T point_camera[3];
        ceres::AngleAxisRotatePoint(rotation_angle_axis, point_global_from_camera_center, point_camera);
        ...
        T r2 = x * x + y * y;
        T rad_distortion = 1. + camera_intrinsics[0]*r2 + camera_intrinsics[1]*r2*r2;
        x *= rad_distortion;
        y *= rad_distortion;
#endif
