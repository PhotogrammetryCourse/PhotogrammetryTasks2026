...
TEST (CeresSolver, HelloWorld1) {
    ...
    // ans:    потому что float и погрешность вычислений
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        T numerator = a * samplePoint[0] + b * samplePoint[1] + c;
        T denominator = ceres::sqrt(a * a + b * b);
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    double scale_r = ideal_line[2] / line_params[2];
    for (int d = 0; d < 3; ++d) line_params[d] *= scale_r;
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // почему они расходятся? - то как мы задали прямую это не единственный способ её задать, можем домножить все коэфициенты на какое-то k, то получим ту же прямую
        // и Ceres solver может найти вместо {0.5, -1.0, 100.0} -> k*{0.5, -1.0, 100.0}
        ...
        // > нормализовать прямую, нам нужен единый масштаб чтобы  a^2 + b^2 = 1
        ...
        // > использовать нормальную форму Гессе x*cos(teta) + y*sin(teta) = phi, тогда автоматически выполнится cos^2(teta) + sin^2(teta) = 1, teta - угол, phi расстояние от начала координат
    ...
    ASSERT_GT(inliers_fraction, 0.99 - outliers_fraction); // TODO 8 раскоментируйте, почему эта проверка падает? как поправить? - эта проверка падает на тестах с outliers потому что они не входят в inliers_fraction. чтобы подправить нужно вычитать процент outliers
    ASSERT_LT(mse, 1.1 * sigma * sigma); // TODO 9 раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит?
    //  попробуйте отладить рассчет mse_inliers_distance в evaluateLine - аналогично с предыдущим, потому что квадратичная ошибка у шума может быть очень большой и они попадают в mse
