...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Потому что это производная функция невязки `10.0 - x[0]` даёт константную производную, равную `-1`.
    // У оригинальной функции там действительно должен быть ноль
...
public:
    ...
    bool operator()(const T* const queryPoint, T* residual) const {
        ...
        residual[0] = a*dx*dx + b*dy*dy + center[2] - queryPoint[2]; // <- тут бага, в плоскости XY найдём расстояние правильно из формулы параболоида
        return true;
    }
protected:
    double center[3];
...
TEST (CeresSolver, HelloWorld2) {
    ...
    // x=10,y=5,z=t ; z-100=2*(x-5)^2+2*(y-10)^2 => t = 200
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        auto [a, b, c] = std::tie(line[0], line[1], line[2]);
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (size_t i = 0; i < 3; i++) {
        line_params[i] *= ideal_line[2] / line_params[2];
    }
    ...
    for (int d = 0; d < 3; ++d) {
       ASSERT_NEAR(line_params[d], ideal_line[d], threshold);
        ...
        // неоднозначно задавать уравнение прямой просто коэффициентами `a,b,c` без условий
        // - пост-обработкой - как-то поправив параметры прямой перед сверкой (при этом не меняя ее положение в пространстве)
        //   так как мы вполне знаем идеальную прямую, то закостылим проверку на `c` <= 0 
        ...
        //   попробуем однозначно задавать прямую так: поделим `a` и `b` на `sqrt(a^2+b^2)` и пусть первый ненулевой из коэффициентов (a, b, c) будет > 0
    ...
    // забыли учесть нагенеренные выбросы
    ...
    // была бага в `calcDistanceToLine2D`, расстояние надо брать по модулю. Первый теств видимо прошёл, потому что получались точки только по одну сторону от прямой
    // Проходит на тесте `CeresSolver.FitLineNoise`, на остальных падает
