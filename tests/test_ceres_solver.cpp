...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Потому что final_jacobian это производная функции невязки x - 10, то есть константная -1
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Ошибка была в ResidualToParaboloid
    }
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    double threshold = 1e-2 * std::max(std::abs(ideal_line[0]), std::max(std::abs(ideal_line[1]), std::abs(ideal_line[2])));
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Так как количество параметров, задающих одну и ту же прямую бесконечное число можем зафиксировать один из них и решить относительно оставшихся двух
        // Также можно нормировать нашу прямую, чтобы привести ее к эталонной
    ...
    // В проверке не учитываем процент выбросов, которыые сгенерировали (поэтому и падает на последних двух тестах)
    ...
    // В evaluateLine расстояние надо брать по модулю
