...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Вообще, мне кажется что сработает, но непонятно зачем он тут нужен, слишком простая задача. Его можно использовать для решения системы J * J^(-1), но явно не тут
    ...
    // Начнем с того что мы то на самом деле сошлись: f(x):  5 -> 1.66644e-08.
    // Во вторых, минимизируем мы loss а не нашу функцию residual.
    // В третьих, у residual функции градиент просто не может быть 0, она же задана как 10 - x, то есть градиент всегда -1
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        //
        // только нвязка для параболоида, прямая содержит эту точку
        //
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    // нормализуем прямые по с = 100, чтобы убрать степень свободы
    ...
    // она не рассчитывает на тесты с аутлайерами, которые заведомо не должны быть зафичены. Нужно учесть их долю
    ...
    // Падает на тестах с аутлайерами. Аутлайеры с отрицательным расстоянием проходят тест трех сигм без модуля и взрывают mse
