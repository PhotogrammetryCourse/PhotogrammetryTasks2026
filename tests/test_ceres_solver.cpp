...
TEST (CeresSolver, HelloWorld1) {
    ...
    // 1: почему результирующая производная не ноль? мы ведь должны были сойтись в минимуме функции 0.5*(10-x)^2
    // Потому что погрешность вычислений
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
       EXPECT_NEAR(point[d], expected_point_solution[d], 1e-4);
        // 3: раскомментируйте^, почему он находит не то что ожидалось?
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // 5 посчитайте единственную невязку - расстояние от нашей точки-замера до текущего состояния прямой (для извлечения корня, помня про T=Jet, нужно использовать ceres::sqrt):
        ...
        residual[0] = (line[0] * samplePoint[0] + line[1] * samplePoint[1] + line[2]) / ceres::hypot(line[0], line[1]);
        return true;
    }
...
class UnitLineNormConstraint {
public:
    template <typename T>
    bool operator()(const T* const line, T* residual) const {
        residual[0] = line[0] * line[0] + line[1] * line[1] - 1.0;
        return true;
    }
};
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    // {
    //     ceres::CostFunction* unit_residual = new ceres::AutoDiffCostFunction<UnitLineNormConstraint, 1, 3>(new UnitLineNormConstraint());
    //     problem.AddResidualBlock(unit_residual, nullptr, line_params);
    // }
    ...
    double ideal_scl = ideal_line[2];
    double ans_scl = line_params[2];
    for (int d = 0; d < 3; ++d) line_params[d] *= ideal_scl / ans_scl; 

    for (int d = 0; d < 3; ++d) {
        ASSERT_NEAR(line_params[d], ideal_line[d], threshold);
        // 7 расскоментируйте сверку найденной прямой и эталонной
        ...
        // 7 поправьте тест так или иначе (хотя бы пост-процессингом)
        // Вариант 1: перенормировать (a, b) в результате так, чтобы он был той же длины, что и ответ, и имел тот же угол по модулю pi
        // Вариант 2: параметризовать (a, b) = (cos phi, sin phi). Тут надо будет взять phi по модулю pi, чтобы избавиться от неоднозначности направления.
        //            И всё равно домножить на -|(a, b)| ответа, потому что он не нормирован и смотрит не туда
        // Вариант 3: добавить в систему ещё 1 условие: (a**2 + b**2 = 1). Ну и всё равно нужна пост-обработка, потому что ответ не нормирован
        // Вариант 4: откуда-то знать правильные нормировку и направление и добавить условия (a**2 + b**2 = 1.25) и (b < 0) (relu(b) = 0)
        // Вариант 5: меняем в предыдущих вариантах нормировку на нормировку по одному коэффициенту прямой (откуда-то зная, что он не ноль)
    }

    // Оцениваем качество идеальной прямой
    double inliers_fraction, mse;
    evaluateLine(points, ideal_line, sigma, inliers_fraction, mse);
    ASSERT_GT(inliers_fraction, 0.99 - outliers_fraction); // 8 раскоментируйте, почему эта проверка падает? как поправить?
    ASSERT_LT(mse, 1.1 * sigma * sigma); // 9 раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
    ...
    if (outliers_fraction == 0 || use_huber) {
        // 10 раскоментируйте обе проверки, почему они падают? в каких тестах? поправьте (в т.ч. подобно тому как было с ослаблением порога выше)
