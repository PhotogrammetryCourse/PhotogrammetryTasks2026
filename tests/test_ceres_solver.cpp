...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Потому что final_jacobian - это производная именно функции невязки 10 - x.
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Была ошибка в формуле в строке 137.
    }

    // если любопытно и хватит времени - можете попросить ceres-solver посчитать якобианы в некоторых точках подобно тому как это делалось в конце теста HelloWorld1
    // и сверить что найденные аналитически на бумажке результаты совпадают (через ASSERT_NEAR)
    {
        double point[3] = {13.0, 5.0, 200.0};
        ...
        line_cost_function->Evaluate(params, &residual, jacobians);
        ASSERT_NEAR(residual, 3.0, 1e-6);
        ASSERT_NEAR(jacobian[0], 1.0, 1e-6);
        ASSERT_NEAR(jacobian[1], 0.0, 1e-6);
        ASSERT_NEAR(jacobian[2], 0.0, 1e-6);

        paraboloid_cost_function->Evaluate(params, &residual, jacobians);
        ASSERT_NEAR(residual, -78.0, 1e-6);
        ASSERT_NEAR(jacobian[0], -32.0, 1e-6);
        ASSERT_NEAR(jacobian[1], 20.0, 1e-6);
        ASSERT_NEAR(jacobian[2], 1.0, 1e-6);
    }
}
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    double aligned_line_params[3];
    double dot_pr = 0, norm2 = 0;
    for (int d = 0; d < 3; ++d) {
        dot_pr += ideal_line[d] * line_params[d];
        norm2 += line_params[d] * line_params[d];
    }
    double scale = dot_pr / norm2;
    for (int d = 0; d < 3; ++d) {
        aligned_line_params[d] = line_params[d] * scale;
    }
    if (outliers_fraction < 1e-4 || use_huber) {
        for (int d = 0; d < 3; ++d) {
            ASSERT_NEAR(aligned_line_params[d], ideal_line[d], threshold);
            ...
            // Потому что, вообще говоря, решений у уравнения найденной прямой бесконечно много с точностью до масштаба этих самых line_params.
            // И нужно либо как-то нормировать и коэффициенты line_params, и коэффициенты ideal_line, либо же (как сделано в коде)
            //   домножить line_params на масштаб, который наилучшим образом приблизит их к ideal_line (что по сути то же самое, что и обычная нормировка, ибо тут
            //   мы по сути проецируем один вектор коэффициентов на другой... однако вариант с обычной нормировкой коэффициентов у меня почему-то сработал хуже,
            //   и пара тестов упало из-за того, что ошибка оказалась больше, чем значение threshold... но не суть :) )
            // Также можно было изначально переформулировать параметры, избавившись от лишней степени свободы,
            //   работая с прямыми в нормальной форме (x*cos(alpha) + y*sin(alpha) = beta).
            //
            // Ещё, кстати, я добавил `if (outliers_fraction < 1e-4 || use_huber)`, ибо как будто, когда есть выбросы и когда не используется huber,
            //   просто минимизируя квадратичную ошибку, мы как будто и не обязаны найти действительно хорошую для ceres::TrivialLoss прямую
            //   из-за оверчувствительности этой ошибки к выбросам.
        }
    }
    ...
    ASSERT_GT(inliers_fraction, 0.99 * (1.0 - outliers_fraction)); // раскоментируйте, почему эта проверка падает? как поправить?
                                                                  // Эээ.. ну как бы.. при outliers_fraction > 0.01 эта проверка по определению не должна проходить xD
                                                                  // Для исправления можно добавить умножение на (1.0 - outliers_fraction) как адекватный компромисс.
    ASSERT_LT(mse, 1.1 * sigma * sigma); // раскомментируйте, почему проверка падает? на каких тестах она падает, на каких проходит? попробуйте отладить рассчет mse_inliers_distance в evaluateLine
                                         // Падали тесты CeresSolver.FitLineNoiseAndOutliers и CeresSolver.FitLineNoiseAndOutliersWithHuberLoss.
                                         // Решилось исправлением бага в evaluateLine, а именно тем, что нужно сравнивать std::abs(dist) с 3 * sigma,
                                         //   а не просто dist, иначе все точки по "отрицательную" сторону прямой автоматом считаются инлайерами.
