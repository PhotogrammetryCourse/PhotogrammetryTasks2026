...
public:
    ...
    bool operator()(const T* const queryPoint, T* residual) const {
        ...
        const T dx = queryPoint[0] - T(center[0]);
        const T dy = queryPoint[1] - T(center[1]);
        const T expected_z = T(a) * dx * dx + T(b) * dy * dy + T(center[2]);
        residual[0] = queryPoint[2] - expected_z;
        return true;
    }
protected:
    double center[3];
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // Знаковое расстояние до прямой: ax + by + c, нормированное на длину нормали.
        const T a = line[0];
        const T b = line[1];
        const T c = line[2];
        const T x = T(samplePoint[0]);
        const T y = T(samplePoint[1]);

        residual[0] = (a * x + b * y + c) / ceres::sqrt(a * a + b * b);
        return true;
    }
protected:
...
void canonicalizeLine(double* line, const double* reference_line) {
    const double norm = std::sqrt(line[0] * line[0] + line[1] * line[1]);
    const double ref_norm = std::sqrt(reference_line[0] * reference_line[0] + reference_line[1] * reference_line[1]);
    ASSERT_GT(norm, 0.0);
    ASSERT_GT(ref_norm, 0.0);

    const double direction_dot = line[0] * reference_line[0] + line[1] * reference_line[1];
    const double sign = direction_dot < 0.0 ? -1.0 : 1.0;
    const double scale = sign * ref_norm / norm;
    for (int d = 0; d < 3; ++d) {
        line[d] *= scale;
    }
}
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    canonicalizeLine(line_params, ideal_line);
    std::cout << "Canonical line: (a=" << line_params[0] << ", b=" << line_params[1] << ", c=" << line_params[2] << ")" << std::endl;

    if (outliers_fraction == 0.0 || use_huber) {
        const double thresholds[3] = {
            0.02,
            0.03,
            outliers_fraction > 0.0 ? 0.2 : 0.1
        };
        for (int d = 0; d < 3; ++d) {
            ASSERT_NEAR(line_params[d], ideal_line[d], thresholds[d]);
        }
    }
    ...
    const double min_expected_inliers_fraction = 0.99 * (1.0 - outliers_fraction);
    const double max_expected_mse = (outliers_fraction > 0.0 ? 1.2 : 1.1) * sigma * sigma;
    ASSERT_GT(inliers_fraction, min_expected_inliers_fraction);
    ASSERT_LT(mse, max_expected_mse);
    ...
    if (outliers_fraction == 0 || use_huber) {
        const double min_fitted_inliers_fraction = 0.99 * (1.0 - outliers_fraction);
        const double max_fitted_mse = (outliers_fraction > 0.0 ? 1.25 : 1.1) * sigma * sigma;
        ASSERT_GT(inliers_fraction, min_fitted_inliers_fraction);
        ASSERT_LT(mse, max_fitted_mse);
    }
}
