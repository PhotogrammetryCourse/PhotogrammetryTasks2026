...
TEST (CeresSolver, HelloWorld1) {
    ...
    // final_jacobian - произведение residual функции 10 - х, она равна -1.
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // residual[0] = queryPoint[2] - (a*dx*dx + b*dy*dy + center[2]) в 137 строке
    }
    ...
    double point_[3] = {12.0, 5.0, 200.0};
    double residual = 1.0;
    ...
    ASSERT_NEAR(residual, -48.0, 1e-6);
    ASSERT_NEAR(jacobian[0], -28.0, 1e-6);
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        // используем: z = tan(alpha)*x + tan(beta)*y + c
        T a = (T) line[0];
        T b = (T) line[1];
        T c = (T) line[2];
        residual[0] = (a*samplePoint[0] + b*samplePoint[1] + c) / ceres::hypot(a, b);
        return true;
    }
protected:
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    if (std::abs(line_params[2]) > 1e-6) {
        const double norm2 = ideal_line[2] / line_params[2];
        line_params[0] *= norm2;
        line_params[1] *= norm2;
        line_params[2] *= norm2;
    }
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // проблема - разные наборы параметров могут описывать одну и ту же прямую.

        // ПОСТОБРАБОТКА:
        // - нормировка коэффициентов
        // - фиксирование масштаба
        // ФОРМУЛИРОВКА ЗАДАЧИ:
        // - Убрать степень свободы путем выражения одного из параметров из другого - 
        // к примеру, z = a*x + b*y + 1, либо через углы.
    }
    ...
    // валится на тестах с outliers, они в inliers_fraction не входят
    ...
    // валится на тестах с outliers. В evaluateLine брался не модуль дистанции -> все отрицательные значения  считались инлаерами
    ...
    if (outliers_fraction == 0 || use_huber) {
        ...
        ASSERT_GT(inliers_fraction, 0.99 - outliers_fraction); // валится на тестах с outliers, они в inliers_fraction не входят
        ASSERT_LT(mse, 1.1 * sigma * sigma); // валится на тестах с outliers. В evaluateLine брался не модуль дистанции -> все отрицательные значения  считались инлаерами
    }
}
