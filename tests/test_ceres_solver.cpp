...
TEST(CeresSolver, HelloWorld1)
{
    ...
    // Параметры и якобианы - это указатели-т.е. список (столько сколько блоков невязки, в нашем случае - 1 блок) из указателей-т.е. списков (каждый такой длинны, сколько параметров в этом отдельном блоке, у нас в единственном блоке
    ...
    std::cout << "grad of residual_squared: " << " -> " << final_jacobian * 2 * final_residual << std::endl;
    // DONE TODO 1: почему результирующая производная не ноль? мы ведь должны были сойтись в минимуме функции 0.5*(10-x)^2
    // ответ: производная и есть final_jacobian * 2 * final_residual,
    //  а не final_jacobian, т.е. производная от residual^2, а не от residual, и именно она должна быть нулем

    // производная от residual (т.е. от 10-x) - это -1, а не 0,
    // а производная от residual^2 - это 2*residual*residual` = 2*(10-x)*(-1) = -2*(10-x),
    // которая в точке x=10 равна нулю, а в окрестности точки x=10 - не равна нулю
...
public:
    ...
    template <typename T> bool operator()(const T* const queryPoint, T* residual) const
    {
...
public:
    ...
    template <typename T> bool operator()(const T* const queryPoint, T* residual) const
    {
        ...
        residual[0] = queryPoint[2] - (a * dx * dx + b * dy * dy + center[2]); /// fix for -- a*dx*dx + b*dy*dy - center[2];
        return true;
    }
...
TEST(CeresSolver, HelloWorld2)
{
    ...
    // return; // DONE TODO 2 удалите эту строку, затем
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // DONE TODO 3: раскомментируйте^, почему он находит не то что ожидалось?
...
public:
    ...
    template <typename T> bool operator()(const T* const line, T* residual) const
    {
        // Блок параметров - line=[a, b, c] - задает прямую вида ax+by+c=0
        // DONE (?) TODO 5 посчитайте единственную невязку - расстояние от нашей точки-замера до текущего состояния прямой (для извлечения корня, помня про T=Jet, нужно использовать ceres::sqrt):
...
void evaluateLineFitting(double sigma, double& fitted_inliers_fraction, double& mean_inliers_distance, double outliers_fraction = 0.0, bool use_huber = false)
{
    ...
    for (size_t i = 0; i < n_points; ++i) {
        ...
        // return; // (DONE) TODO 6 удалите этот return сразу после выполнения TODO 5
...
void evaluateLine(const std::vector<double_2>& points, const double* line, double sigma, double& fitted_inliers_fraction, double& mse_inliers_distance)
{
    size_t inliers = 0;
    double sum_sq = 0.0;
    ...
    double inlier_fraction = static_cast<double>(inliers) / points.size();
    double mse = (inliers > 0) ? (sum_sq / inliers) : 0.0;
    fitted_inliers_fraction = inlier_fraction;
    mse_inliers_distance = mse;
}
