...
TEST (CeresSolver, HelloWorld1) {
    ...
    // В минимуме зануляется производная стоимости 0.5 * residual^2, а не производная невязки residual = 10 - x
...
TEST (CeresSolver, HelloWorld2) {
    ...
    // return; //  2 удалите эту строку, затем
...
public:
    ...
    bool operator()(const T* const line, T* residual) const {
        ...
        T numerator = line[0] * T(samplePoint[0]) + line[1] * T(samplePoint[1]) + line[2];
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    for (size_t i = 0; i < n_points; ++i) {
        ...
        // return; // 6 удалите этот return сразу после выполнения 5
    ...
    double comparable_line[3];
    double scale = ideal_line[1] / line_params[1];
    for (int d = 0; d < 3; ++d) {
        comparable_line[d] = line_params[d] * scale;
    }
    ...
    if (outliers_fraction > 0.0 && !use_huber) {
        threshold *= 3.0; // ослабляем порог если есть выбросы и мы к ним не устойчивы (не робастны за счет loss-функции (функции потерь) Huber-а)
    }
    if (outliers_fraction == 0.0 || use_huber) {
        for (int d = 0; d < 3; ++d) {
            ASSERT_NEAR(comparable_line[d], ideal_line[d], threshold);
        }
    }
    ...
    const double expected_min_inliers_fraction = 0.99 * (1.0 - outliers_fraction);
    ASSERT_GT(inliers_fraction, expected_min_inliers_fraction);
