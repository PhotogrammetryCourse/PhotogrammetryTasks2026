...
TEST (CeresSolver, HelloWorld1) {
    ...
    // Вывод:
    // iter      cost      cost_change  |gradient|   |step|    tr_ratio  tr_radius  ls_iter  iter_ti
    //    0  1.250000e+01    0.00e+00    5.00e+00   0.00e+00   0.00e+00  1.00e+04        0    6.60e-
    //    1  1.249750e-07    1.25e+01    5.00e-04   0.00e+00   1.00e+00  3.00e+04        1    7.01e-
    //    2  1.388518e-16    1.25e-07    1.67e-08   5.00e-04   1.00e+00  9.00e+04        1    1.91e-
    // Ceres Solver Report: Iterations: 3, Initial cost: 1.250000e+01, Final cost: 1.388518e-16, Ter
    // x:     5 -> 10
    // f(x):  5 -> 1.66644e-08
    // f'(x): -1 -> -1 -- initial_jacobian -> final_jacobian
    // значение не равно 0, поскольку выводим якобиан, т.е. производную residual, а не производную целевой функции
    // оно вообще не обязано быть 0 в экстремуме
...
TEST (CeresSolver, HelloWorld2) {
    ...
    for (int d = 0; d < 3; ++d) {
        ...
        // Из-за ResidualToParaboloid::operator()
    }
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    double threshold = 1e-2 * std::max({std::abs(ideal_line[0]), std::abs(ideal_line[1]), std::abs(ideal_line[2])});
    if (outliers_fraction > 0.0 && !use_huber) {
        threshold *= 20.0; // ослабляем порог если есть выбросы и мы к ним не устойчивы (не робастны за счет loss-функции (функции потерь) Huber-а)
    }

    auto normalize = [](double* l) {
        double norm = sqrt(std::pow(l[0], 2) + std::pow(l[1], 2));
    ...
    double ideal[3] = { ideal_line[0], ideal_line[1], ideal_line[2] };
    double found[3] = { line_params[0], line_params[1], line_params[2] };

    normalize(ideal);
    normalize(found);

    for (int d = 0; d < 3; ++d) {
        ASSERT_NEAR(found[d], ideal[d], threshold);
