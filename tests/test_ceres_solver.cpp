...
public:
    ...
    bool operator()(const T* const queryPoint, T* residual) const {
        ...
        residual[0] = ceres::abs(a*dx*dx + b*dy*dy + center[2] - queryPoint[2]);
        return true;
    }
protected:
    double center[3];
...
void evaluateLineFitting(double sigma, double &fitted_inliers_fraction, double &mean_inliers_distance, double outliers_fraction=0.0, bool use_huber=false) {
    ...
    // double line_params_unique[2] = {1.0, 1.0};
    ...
    // line_params[0] = line_params_unique[0];
    // line_params[1] = line_params_unique[1];
    // line_params[2] = 100;
    line_params[0] = 100 * line_params[0] / line_params[2];
    line_params[1] = 100 * line_params[1] / line_params[2];
    line_params[2] = 100;

    // double total_error2 = 0;
    // double total_error2_ideal = 0;
    // for (size_t i = 0; i < n_points; ++i) {
    //     total_error2 += std::pow(calcDistanceToLine2D(points[i][0], points[i][1], line_params), 2);
    //     total_error2_ideal += std::pow(calcDistanceToLine2D(points[i][0], points[i][1], ideal_line), 2);
    // }
    // total_error2 *= 0.5;
    // total_error2_ideal *= 0.5;
    // std::cout << "Test: " << total_error2 << ", " << total_error2_ideal << std::endl;
