...
{
    // запустите test_mesh_min_cut/DepthMapsToPointClouds и убедитесь что облака точек построенные по картам глубины выглядят правдоподобно
    ...
    for (size_t ci = 0; ci < dataset.ncameras; ++ci) {
        ...
        phg::buildPoints(depth_maps[ci], dataset.cameras_imgs[ci], dataset.cameras_P[ci], dataset.calibration, points, radiuses, normals, colors);
...
{
    // запустите test_mesh_min_cut/FromAllDepthMaps и убедитесь что полигональные модели построенные по картам глубины выглядят правдоподобно
