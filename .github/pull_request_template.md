...
Попробовал несколько значений, ожидаемо при выставлении большего значения падает детализация и наоборот, потому что мы начинаем мержить больше точек. Вроде без интересностей, больше всего понравилось значение 0.5. Модель становится немного менее шипастой чем дефолтное 0.1, но все еще довольно детальной в отличие от 1.
...
Я если честно вообще почти не замечал разницы при реализации, единственное что удалось заметить это изменение от MERGE_THRESHOLD_RADIUS_KOEF, но оно было вполне ожидаемым.
